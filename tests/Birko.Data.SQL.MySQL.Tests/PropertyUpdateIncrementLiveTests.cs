...
/// <summary>
/// TASK-498 — <c>PropertyUpdate.Increment</c> / <c>Decrement</c> against a live MySQL; the live twin of
/// <c>Birko.Data.SQL.SqLite.Tests.PropertyUpdateIncrementEndToEndTests</c>.
/// </summary>
/// <remarks>
/// <para>
/// The decimal column declares its precision because a bare <c>DECIMAL</c> is <c>DECIMAL(10,0)</c> here and
...
{
    private const string CounterTable = "MyPropertyUpdateIncrement";
    ...
    private static AsyncMySQLStore<Counter> NewStore()
    {
        var store = new AsyncMySQLStore<Counter>();
        store.SetSettings(Settings());
        return store;
    }

    private static async Task<(AsyncMySQLStore<Counter> Store, Guid Target, Guid Bystander)> SeedAsync()
    {
        Exec($"DROP TABLE IF EXISTS `{CounterTable}`");
        new MySQLConnector(Settings()).CreateTable(new[] { typeof(Counter) });
    ...
    private static async Task<Counter> ReadAsync(AsyncMySQLStore<Counter> store, Guid guid)
