...
- `AsyncJsonSyncKnowledgeStoreTests` — last-sync-time Set/Get round-trip + scope isolation + null/empty cases + `CreateKnowledgeItem` field derivation, against a real temp-file XML store. Also exercises the CR-M162 single-bulk-write fix (all matching items updated).
