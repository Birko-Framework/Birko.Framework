...
/// <remarks>
/// <para>
/// A Raven index is a map/reduce definition rather than a field list, so this builder cannot express one. IIndexManager.CreateAsync can, through a Map/Maps/Reduce entry in IndexDefinition.Properties, which RavenDBIndexManager honours.
/// </para>
/// <para>
...
public class IndexBuilderRefusalTests
{
    private static IIndexBuilder Builder()
        => new RavenDBSchemaBuilder(new DocumentStore { Urls = new[] { "http://localhost:8080" }, Database = "probe" })
