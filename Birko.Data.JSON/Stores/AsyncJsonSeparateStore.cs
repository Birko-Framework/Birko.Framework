...
namespace Birko.Data.JSON.Stores
{
    ...
    public class AsyncJsonSeparateStore<T> : AsyncJsonStore<T>
        where T : Models.AbstractModel
    {
        ...
        /// </summary>
        protected Dictionary<Guid, string> _files = null!;
        ...
        /// <summary>
        /// Initializes a new instance of the AsyncJsonSeparateStore class.
        /// </summary>
        public AsyncJsonSeparateStore() : base()
        {
            ...
            foreach (var item in _items)
            {
                if (!_files.ContainsKey(item.Key))
                {
                    var fileName = _settings.Name.Contains('*')
                        ...
                        : $"{_settings.Name}-{item.Key.ToString("D")}";
