...
> ⚠ **This file is not auto-loaded.** [CLAUDE.md](CLAUDE.md) § Conventions lists every rule's opening
> statement and points here. Read the relevant entry **in full** before changing code it covers — these are
> not background reading. Every rule exists because the defect it describes shipped at least once, and
> several exist because it shipped twice after the first fix was reasoned about instead of measured.

Rules are in their original order; `CLAUDE.md` indexes them 1—63.
