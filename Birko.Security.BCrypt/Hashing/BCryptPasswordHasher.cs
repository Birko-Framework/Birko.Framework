...
namespace Birko.Security.BCrypt.Hashing;
...
/// The default work factor of 12 provides a good balance between security and performance (~250ms per hash).
/// </remarks>
public class BCryptPasswordHasher : IPasswordHasher
{
    /// <summary>Minimum allowed work factor.</summary>
    public const int MinWorkFactor = 4;

    /// <summary>Maximum allowed work factor.</summary>
    public const int MaxWorkFactor = 31;

    /// <summary>Default work factor (2^12 = 4096 iterations).</summary>
    public const int DefaultWorkFactor = 12;

    private readonly int _workFactor;

    /// <summary>
    /// Creates a new BCrypt password hasher with the specified work factor.
    /// </summary>
    /// <param name="workFactor">
    /// The BCrypt work factor (cost parameter). The number of iterations is 2^workFactor.
    /// Must be between <see cref="MinWorkFactor"/> and <see cref="MaxWorkFactor"/>. Default: 12.
    /// </param>
    public BCryptPasswordHasher(int workFactor = DefaultWorkFactor)
    {
        if (workFactor < MinWorkFactor || workFactor > MaxWorkFactor)
            throw new ArgumentOutOfRangeException(nameof(workFactor),
                $"Work factor must be between {MinWorkFactor} and {MaxWorkFactor}");
        _workFactor = workFactor;
    }

    /// <summary>
    /// Gets the configured work factor.
    /// </summary>
    ...
    public bool Verify(string password, string hashedPassword)
    {
        ...
        // Extract the salt portion (first 29 characters: $2a$XX$ + 22 chars of salt)
        var salt = hashedPassword[..29];
    ...
    /// <summary>
    /// Checks whether a work factor upgrade is needed for a previously hashed password.
    /// Returns true if the hash was created with a lower work factor than the current setting.
    /// </summary>
    public bool NeedsRehash(string hashedPassword)
    {
        ...
        var costStr = hashedPassword[4..6];
        if (int.TryParse(costStr, out var cost))
            return cost < _workFactor;

        return true;
    }

    #region BCrypt Implementation

    // BCrypt alphabet (different from standard Base64)
    private const string BCryptBase64 = "./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";

    // "OrpheanBeholderScryDoubt" in raw bytes — the magic plaintext that BCrypt encrypts
    private static readonly byte[] BCryptCiphertext =
    {
    ...
    // Blowfish S-boxes (initial values from pi)
    ...
    private static readonly uint[] S_orig = GenerateInitialSBox();

    private static uint[] GenerateInitialSBox()
    {
        // Standard Blowfish S-box initial values (1024 entries from digits of pi)
        // This is the full specification from Bruce Schneier's Blowfish
        var s = new uint[1024];
        var piDigits = new uint[]
        {
            // S-box 0
        ...
        Array.Copy(piDigits, s, piDigits.Length);
        return s;
    }
    ...
    private static string GenerateSalt(int workFactor)
    {
        ...
        return $"$2a${workFactor:D2}${EncodeBCryptBase64(saltBytes, 16)}";
    }

    private static string HashPassword(string password, string salt)
    {
        // Parse salt
        if (salt.Length < 29 || salt[0] != '$')
            throw new ArgumentException("Invalid BCrypt salt format", nameof(salt));

        var costStr = salt[4..6];
        if (!int.TryParse(costStr, out var cost) || cost < MinWorkFactor || cost > MaxWorkFactor)
            throw new ArgumentException("Invalid BCrypt cost factor", nameof(salt));

        var saltEncoded = salt[7..29]; // 22 characters of BCrypt-Base64 encoded salt
        var saltBytes = DecodeBCryptBase64(saltEncoded, 16);

        // Convert password to bytes (UTF-8, null-terminated, max 72 bytes)
        var passwordBytes = Encoding.UTF8.GetBytes(password + "\0");
        if (passwordBytes.Length > 72)
        {
            var trimmed = new byte[72];
            Array.Copy(passwordBytes, trimmed, 72);
            passwordBytes = trimmed;
        }

        // Run expensive key setup (EksBlowfish)
        var P = new uint[18];
        var S = new uint[1024];
        Array.Copy(P_orig, P, 18);
        Array.Copy(S_orig, S, 1024);

        // Expand key with salt and password
        EksBlowfishSetup(P, S, saltBytes, passwordBytes, cost);

        // Encrypt "OrpheanBeholderScryDoubt" 64 times
        var ctext = new uint[6];
        ctext[0] = (uint)(BCryptCiphertext[0] << 24 | BCryptCiphertext[1] << 16 | BCryptCiphertext[2] << 8 | BCryptCiphertext[3]);
        ctext[1] = (uint)(BCryptCiphertext[4] << 24 | BCryptCiphertext[5] << 16 | BCryptCiphertext[6] << 8 | BCryptCiphertext[7]);
        ctext[2] = (uint)(BCryptCiphertext[8] << 24 | BCryptCiphertext[9] << 16 | BCryptCiphertext[10] << 8 | BCryptCiphertext[11]);
        ctext[3] = (uint)(BCryptCiphertext[12] << 24 | BCryptCiphertext[13] << 16 | BCryptCiphertext[14] << 8 | BCryptCiphertext[15]);
        ctext[4] = (uint)(BCryptCiphertext[16] << 24 | BCryptCiphertext[17] << 16 | BCryptCiphertext[18] << 8 | BCryptCiphertext[19]);
        ctext[5] = (uint)(BCryptCiphertext[20] << 24 | BCryptCiphertext[21] << 16 | BCryptCiphertext[22] << 8 | BCryptCiphertext[23]);

        for (int i = 0; i < 64; i++)
        {
            for (int j = 0; j < 6; j += 2)
            {
                BlowfishEncrypt(P, S, ref ctext[j], ref ctext[j + 1]);
            }
        }
        ...
        for (int i = 0; i < 6; i++)
        {
            hashBytes[i * 4] = (byte)(ctext[i] >> 24);
            hashBytes[i * 4 + 1] = (byte)(ctext[i] >> 16);
            hashBytes[i * 4 + 2] = (byte)(ctext[i] >> 8);
            hashBytes[i * 4 + 3] = (byte)(ctext[i]);
        }

        // BCrypt only uses 23 bytes of the hash output
    ...
    {
        uint a = S[(x >> 24) & 0xFF];
        uint b = S[256 + ((x >> 16) & 0xFF)];
        uint c = S[512 + ((x >> 8) & 0xFF)];
        uint d = S[768 + (x & 0xFF)];
        return ((a + b) ^ c) + d;
    }
    ...
    private static string EncodeBCryptBase64(byte[] data, int length)
    {
        ...
        while (off < length)
        {
            uint c1 = off < length ? data[off++] : (uint)0;
            sb.Append(BCryptBase64[(int)((c1 >> 2) & 0x3F)]);
            c1 = (c1 & 0x03) << 4;
            if (off >= length) { sb.Append(BCryptBase64[(int)(c1 & 0x3F)]); break; }
            uint c2 = data[off++];
            c1 |= (c2 >> 4) & 0x0F;
            sb.Append(BCryptBase64[(int)(c1 & 0x3F)]);
            c1 = (c2 & 0x0F) << 2;
            if (off >= length) { sb.Append(BCryptBase64[(int)(c1 & 0x3F)]); break; }
            uint c3 = data[off++];
            c1 |= (c3 >> 6) & 0x03;
            sb.Append(BCryptBase64[(int)(c1 & 0x3F)]);
            sb.Append(BCryptBase64[(int)(c3 & 0x3F)]);
        }
        return sb.ToString();
    }

    private static byte[] DecodeBCryptBase64(string encoded, int maxBytes)
    {
